	...
	return (
		<>
			<Grid>
				{products?.map((product) => (
					<Grid.Col mah={800} key={product.id} span={{ base: 12, md: 6, lg: 3 }}>
						...
						>
							<Card.Section>
								<Image src={product.image} alt={product.model} />
							</Card.Section>
							...
							{isProductAddedToCart[product.id] ? (
								<Group justify="stretch" align="center" gap={5}>
									...
									<NumberInput
										value={cart.find((item) => item.product === product.id)?.quantity}
										onChange={(e) => handleQuantityChange(e, product.id)}
									/>
									<Button onClick={(e) => handleIncreaseQuantity(e, product.id)}>+</Button>
								</Group>
							) : (
								<Button onClick={(e) => handleAddToCart(e, product)} color="purple" fullWidth mt="md" radius="md">
									Add To Cart
								</Button>
							)}
						</Card>
			...
			<Group gap={5} justify="center">
				...
				<Select value={limit} onChange={setLimit} placeholder="Set Limit" data={["10", "20", "30", "40", "50"]} />
			</Group>
			<Space h="xl" />
		</>
	);
